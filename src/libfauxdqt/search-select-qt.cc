...
private:
    ...
    void copy_selected_to_new (int playlist, bool jumptoit);
    void focus_first_selected_entry (int playlist, bool jumptoit);
    void search ();
...
SearchSelectDialog::SearchSelectDialog (QWidget * parent) : QDialog (parent)
{
    ...
    m_checkbox_jumptoit = new QCheckBox (_("Jump right to it (unless new playlist)"));
    ...
    grid->addWidget (m_checkbox_jumptoit, 8, 0, 1, 2);
...
void SearchSelectDialog::copy_selected_to_new (int playlist, bool jumptoit)
{
    ...
    auto new_list = aud_playlist_new ();
...
void SearchSelectDialog::focus_first_selected_entry (int playlist, bool jumptoit)
{
    int entries = aud_playlist_entry_count (playlist);
    for (int i = 0; i < entries; i ++)
    {
        ...
        {
            aud_playlist_set_focus (playlist, i);
            if (jumptoit)
                aud_playlist_set_position (playlist, i);
            return;
        }
    }
}
...
void SearchSelectDialog::search ()
{
    ...
    if (m_checkbox_newplaylist->isChecked ())
        copy_selected_to_new (playlist, m_checkbox_jumptoit->isChecked ());
    else
    {
        /* set focus on the first entry found */
        focus_first_selected_entry (playlist, m_checkbox_jumptoit->isChecked ());
